--[[
    WARNING: Heads up! Fixed & Fully Async UnpatchaBomb Pro.
    - Background async loading for dictionaries (no more freeze/crash on start)
    - Safe fallback if GitHub fails
...
-- Базовый надежный список слов на случай, если ссылки с GitHub недоступны
local FallbackDefaultWords = {
    "apple", "application", "banana", "band", "bandwidth", "cat", "catch", "caterpillar",
    "dog", "dodge", "elephant", "engine", "fire", "firefox", "game", "gamer", "house",
    "internet", "java", "javascript", "keyboard", "laptop", "mouse", "network", "orange",
    "player", "prompt", "queue", "router", "script", "table", "user", "value", "window",
    "pseudopseudohypoparathyroidism", "floccinaucinihilipilification", "antidisestablishmentarianism"
}

-- Глобальные таблицы словарей и индексов
local primaryRaw = {}
local fallbackRaw = {}
...
-- Асинхронная загрузка словарей в фоне
task.spawn(function()
    print("⏳ [UnpatchaBomb]: Loading dictionaries in background...")
    ...
    if not successEnglish or type(ENGLISH_WORDS) ~= "table" then ENGLISH_WORDS = FallbackDefaultWords end
    ...
    if not successWordList or type(WordList) ~= "table" then WordList = FallbackDefaultWords end
    ...
    if not successWordListTwo or type(WordList_Two) ~= "table" then WordList_Two = FallbackDefaultWords end
    ...
    -- Заполняем основной пул (10k)
    for _, v in pairs(WordList) do primaryRaw[#primaryRaw + 1] = v end
    for _, v in pairs(WordList_Two) do primaryRaw[#primaryRaw + 1] = v end
    for _, v in pairs(WordList_Three) do primaryRaw[#primaryRaw + 1] = v end

    -- Заполняем мега-пул (286k фоллбэк)
    for _, v in pairs(ENGLISH_WORDS) do fallbackRaw[#fallbackRaw + 1] = v end
    for _, v in pairs(WordList) do fallbackRaw[#fallbackRaw + 1] = v end
    for _, v in pairs(WordList_Two) do fallbackRaw[#fallbackRaw + 1] = v end
    for _, v in pairs(WordList_Three) do fallbackRaw[#fallbackRaw + 1] = v end
    ...
    print("✅ [UnpatchaBomb]: Dictionaries loaded and indexed successfully!")
end)
...
-- Безопасная функция поиска (ждет окончания индексации, если тащится из сети)
...
MakeIntroLabel(0.54, 0.08, "PRO EDITION (LAG-FREE)")
...
local function ApplyTheme(theme)
    ...
    if theme == "Patriotic" then
        ...
        HowToUseText.Text = "Patriotic UI Active (Indexed)"
        T1.TextColor3 = Color3.fromRGB(255, 255, 255)
        ...
        HowToUseText.TextColor3 = Color3.fromRGB(140, 140, 140)
        HowToUseText.Text = "Modern UI Active (Indexed)"
        T1.TextColor3 = Color3.fromRGB(255, 255, 255)
        T2.TextColor3 = Color3.fromRGB(200, 200, 200)
        MinimizeButton.TextColor3 = Color3.fromRGB(255, 255, 255)
    end
