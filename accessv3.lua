-- =========================================================================
--  BRO-PIXEL | WORD BOMB (EXACT SOLARIS 1:1 - OBFUSCATION SAFE)
-- =========================================================================

local _G_ENV = (type(getgenv) == "function" and getgenv()) or _G or {}
local game = rawget(_G_ENV, "game") or rawget(_G, "game") or game
local task = rawget(_G_ENV, "task") or rawget(_G, "task") or task
...
local HttpService = game.HttpService or game:GetService("HttpService")
...
local TweenService = game.TweenService or game:GetService("TweenService")
...
-- Тумблер Jitter + Ползунок секунд
...
-- === НАДЁЖНАЯ ПРИВЯЗКА И ИНДЕКСАЦИЯ СЛОВАРЯ ===
...
local function applyLoadedDatabase(words, index, common, count)
    globalWordsList = words
    PromptIndex = index
    CommonWordsMap = common
    for i = 1, #words do
        FastWordsMap[words[i]] = true
    end
    dictStatusLabel:SetText(tostring(count or #words) .. " (Ready)")
end

local function syncDatabase()
    local db = rawget(_G_ENV, "WordBomb_Database")
    if db and db.Ready and #db.WordsList > 0 then
        applyLoadedDatabase(db.WordsList, db.PromptIndex, db.CommonMap, db.TotalWords)
        return
    end

    dictStatusLabel:SetText("Loading Dict...")
    task_spawn(function()
        local startWait = os_clock()
        while os_clock() - startWait < 3.5 do
            local currentDb = rawget(_G_ENV, "WordBomb_Database")
            if currentDb and currentDb.Ready and #currentDb.WordsList > 0 then
                applyLoadedDatabase(currentDb.WordsList, currentDb.PromptIndex, currentDb.CommonMap, currentDb.TotalWords)
                return
            end
            task_wait(0.1)
        end
        ...
        task_spawn(function()
            local ok, rawCommon = pcall(function() return game:HttpGet(commonUrl) end)
            if ok and rawCommon then
                for word in rawCommon:gmatch("[^\r\n]+") do
                    word = word:gsub("%s+", ""):lower()
                    if #word >= 2 then CommonWordsMap[word] = true end
                end
            end
        end)

        local success, raw = pcall(function() return game:HttpGet(dictUrl) end)
        if not success or not raw then
            dictStatusLabel:SetText("Dict Error!")
            return
        end
        ...
        for word in raw:gmatch("[^\r\n]+") do
            ...
            if len >= 2 then
                ...
                FastWordsMap[word] = true
                table_clear(seenSub)

                for l = 2, 3 do
                    for i = 1, len - l + 1 do
                        local sub = string_sub(word, i, i + l - 1)
        ...
        applyLoadedDatabase(globalWordsList, PromptIndex, CommonWordsMap, total)
    end)
end

syncDatabase()

-- Мгновенная валидация словаря без линейного поиска по массивам
local function isValidDictionaryWord(word)
    if not word or #word < 2 then return false end
    return CommonWordsMap[word] == true or FastWordsMap[word] == true
end
...
-- === ПРЯМОЙ ДОСТУП К UI БЕЗ РЕКУРСИВНОГО СКАНИРОВАНИЯ ===
local cachedGameUI = nil
local cachedTypebox = nil

local function getGameUI()
    if cachedGameUI and cachedGameUI.Parent then return cachedGameUI end
    local playerGui = LocalPlayer and LocalPlayer:FindFirstChildOfClass("PlayerGui")
    if not playerGui then return nil end
    cachedGameUI = playerGui:FindFirstChild("GameUI") or playerGui:FindFirstChild("GameUI", true)
    return cachedGameUI
end
...
local function GetTurn()
    ...
    local typeBox = cachedTypebox and cachedTypebox.Parent and cachedTypebox or gameUI:FindFirstChild("Typebox", true)
    if typeBox then
        cachedTypebox = typeBox
        if typeBox.Visible then return LocalPlayer.UserId end
    end
    return nil
end
...
local function getGameTextBox()
    local gameUI = getGameUI()
    if not gameUI then return nil end
    if cachedTypebox and cachedTypebox.Parent and cachedTypebox:IsA("TextBox") then
...
-- === ОПТИМИЗИРОВАННЫЙ СБОР БУКВ ПРОМПТА ===
local framesBuffer = {}
local function GetLetters()
    ...
    local textFrame = gameUI:FindFirstChild("TextFrame") or gameUI:FindFirstChild("TextFrame", true)
    if textFrame then
    ...
    local promptLbl = gameUI:FindFirstChild("PromptLabel") or gameUI:FindFirstChild("PromptLabel", true)
...
local function getInfoTable()
    local s, r = pcall(function()
        local getupvaluesFunc = (type(debug_getupvalues) == "function" and debug_getupvalues) or (type(getupvalues) == "function" and getupvalues)
        local getinfoFunc = (type(debug_getinfo) == "function" and debug_getinfo) or (type(debug) == "table" and type(debug.getinfo) == "function" and debug.getinfo)
...
local function UpdatePassivePromptDisplay()
    ...
    task_defer(function()
        ...
        if raw and raw ~= "" then
            ...
            local candidates = PromptIndex[clean]
            local count = 0
            if candidates then
                for i = 1, #candidates do
                    if #candidates[i] <= lettercap and not sessionUsedWords[candidates[i]] then
                        count = count + 1
                    end
                end
            end
            solutionsLabel:SetText(tostring(count))
...
-- === УЛЬТРА-ОПТИМИЗИРОВАННЫЙ СЕТЕВОЙ БЛОК ===
        ...
        gameEvent.OnClientEvent:Connect(function(...)
            ...
            task_spawn(function()
                ...
                if rawType == "typingevent" then
                    ...
                    local text = args[4] or args[3] or args[#args]
                    if pId and pId ~= LocalPlayer.UserId and type(text) == "string" and #text > 0 then
                        lastBuffers[pId] = text
                        lastPossessor = pId
                    end
                elseif rawType == "changepossessor" then
                    ...
                    if stealEnabled and pId and word and LocalPlayer and pId ~= LocalPlayer.UserId then
                        local cleanWord = tostring(word):gsub("%s+", ""):lower()
                        if isValidDictionaryWord(cleanWord) then
                            sessionUsedWords[cleanWord] = true
                            local plr = Players:GetPlayerByUserId(pId)
                            local pName = plr and plr.Name or ("User_" .. tostring(pId))
                            ...
                            lastStolenLabel:SetText(pName .. " (" .. cleanWord:upper() .. ")")
                            stolenPoolLabel:SetText(#stolenReadyPool .. " words")
                        end
                        lastBuffers[pId] = nil
                    end
