--[[
    WARNING: Heads up! UnpatchaBomb Classic + Render 286k Fallback Edition
    - Original UnpatchaBomb search logic & classic word pools (Marsqq + GitHub links)
    - Automatic fallback to your 286k Render dictionary on "No word found"
    - Lag-free UI & full protection against repeats
]]
...
-- Функция поиска: Сначала в классике, если пусто — фоллбэк в 286к с Render
local function FindWordAuto(l)
    ...
    -- 1. Ищем в классическом вордлисте
    local found = searchList(FinalWordList)

    -- 2. Если в классике нет (No word found), идем в фоллбэк 286к с Render
    ...
    if found then 
        ...
        table.insert(UsedWords, upper)
        return upper 
    end
    return nil
end

-- Безопасный метод получения хода и букв без лагающего getgc()
...
MakeIntroLabel(0.54, 0.08, "PRO EDITION (FALLBACK)")
...
local HowToUseText = MakeLabel(MainFrame, 0.75, 0.1,
    "Classic + Render Fallback Active\nMin/Max len set via textboxes")
