...
-- === AUTH & API CONFIG FOR RENDER FALLBACK ===
...
-- 1. Основной мега-список (классика)
local MegaList = {}
local function merge(tbl)
    if type(tbl) ~= "table" then return end
    for _, v in pairs(tbl) do
        if type(v) == "string" then MegaList[#MegaList + 1] = v end
    end
end

if type(ENGLISH_WORDS) == "table" then merge(ENGLISH_WORDS) end
merge(WordList)
merge(WordList_Two)
merge(WordList_Three)

local seen = {}
local UniqueMegaList = {}
for _, word in ipairs(MegaList) do
    local lower = string.lower(word)
    if not seen[lower] then
        seen[lower] = true
        UniqueMegaList[#UniqueMegaList + 1] = word
    end
end
local FinalWordList = UniqueMegaList

-- 2. Фоллбэк список (286к с Render бэкенда)
local RenderMegaList = {}
if dictUrl then
    task.spawn(function()
        local ok, raw = pcall(function() return game:HttpGet(dictUrl) end)
        if ok and raw then
            for word in raw:gmatch("[^\r\n]+") do
                word = word:gsub("%s+", "")
                if #word >= 2 then
                    table.insert(RenderMegaList, word)
                end
            end
        end
    end)
...
local function FindWordAuto(l)
    ...
    local function searchList(list)
        ...
        for _, v in ipairs(list) do
            ...
            if string.find(lv, lowerL, 1, true)
    ...
    local found = searchList(FinalWordList)
    if not found and #RenderMegaList > 0 then
        found = searchList(RenderMegaList)
    end

    if found then 
        local upper = string.upper(found)
        return upper 
    end
    return nil
end
...
TopSearchOverlay.BackgroundTransparency = 0.3
TopSearchOverlay.BackgroundColor3 = Color3.fromRGB(15, 15, 20)
TopSearchOverlay.Position = UDim2.new(0.5, -120, 0, 12)
TopSearchOverlay.Size = UDim2.new(0, 240, 0, 36)
TopSearchOverlay.Font = Enum.Font.GothamBold
TopSearchOverlay.Text = "SEARCHING..."
TopSearchOverlay.TextColor3 = Color3.fromRGB(0, 255, 120)
TopSearchOverlay.TextSize = 16
Instance.new("UICorner", TopSearchOverlay).CornerRadius = UDim.new(0, 6)
    ...
    while task.wait(0.1) do
        pcall(function()
            ...
            if prompt and prompt ~= "" then
                local foundWord = FindWordAuto(prompt)
                if foundWord then
                    TopSearchOverlay.Text = "💡 " .. foundWord
                    TopSearchOverlay.TextColor3 = Color3.fromRGB(0, 255, 120)
                else
                    TopSearchOverlay.Text = "❌ No word found"
                    TopSearchOverlay.TextColor3 = Color3.fromRGB(255, 50, 50)
                end
            else
                TopSearchOverlay.Text = "⏳ Waiting prompt..."
                TopSearchOverlay.TextColor3 = Color3.fromRGB(180, 180, 180)
            end
        end)
    end
end)
...
task.spawn(function()
    while LoadingLabel and LoadingLabel.Parent do
        LoadingLabel.Text = "Loading."  ; task.wait(0.15)
        LoadingLabel.Text = "Loading.." ; task.wait(0.15)
        LoadingLabel.Text = "Loading..."; task.wait(0.15)
    end
end)
...
local function ApplyTheme(theme)
    ...
    if theme == "Patriotic" then
        ...
        HowToUseText.Text = "Patriotic UI Active"
        T1.TextColor3 = Color3.fromRGB(255, 255, 255)
        ...
        HowToUseText.TextColor3 = Color3.fromRGB(140, 140, 140)
        HowToUseText.Text = "Modern UI Active"
        T1.TextColor3 = Color3.fromRGB(255, 255, 255)
        T2.TextColor3 = Color3.fromRGB(200, 200, 200)
        MinimizeButton.TextColor3 = Color3.fromRGB(255, 255, 255)
    end
