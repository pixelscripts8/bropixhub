--[[
    WARNING: Heads up! UnpatchaBomb Pro Edition (All Old Links + Render + Indexed Mega Pool)
    - Full old word lists via GitHub links (ENGLISH_WORDS + WordList + WordList_Two)
    - Fallback mega pool via Render API (286k words)
    - Full removal of laggy getgc() & cleaned special characters
]]
...
-- Универсальная функция построения индекса с очисткой от спецсимволов
local function buildIndex(sourceTbl, indexTbl)
    if type(sourceTbl) ~= "table" then return end
    local seen = {}
    for _, word in ipairs(sourceTbl) do
        if type(word) == "string" then
            local clean = string.lower(word):gsub("[%-%'%s+]", "")
            if #clean >= 2 and not seen[clean] then
                seen[clean] = true
                local len = #clean
                local seenSub = {}
                for l = 2, math.min(len, 4) do
                    for i = 1, len - l + 1 do
                        local sub = string.sub(clean, i, i + l - 1)
...
-- Асинхронная сборка всех старых ссылок + Render 286k
task.spawn(function()
    print("⏳ [UnpatchaBomb]: Loading all old word lists and Render database...")

    local primaryRaw = {}
    local fallbackRaw = {}

    -- 1. Старая ссылка на english words
    local successEnglish, ENGLISH_WORDS = pcall(function()
        return loadstring(game:HttpGet("https://raw.githubusercontent.com/MarsQQ/Unpatchabomb/master/unpatchabomb_english_words", true))()
    end)
    if successEnglish and type(ENGLISH_WORDS) == "table" then
        for _, v in pairs(ENGLISH_WORDS) do table.insert(primaryRaw, v) table.insert(fallbackRaw, v) end
    end

    -- 2. Старая ссылка на words.lua
    local successWordList, WordList = pcall(function()
        return loadstring(game:HttpGet("https://raw.githubusercontent.com/jjengu/scripts/refs/heads/main/wordbomb/words.lua"))()
    end)
    if successWordList and type(WordList) == "table" then
        for _, v in pairs(WordList) do table.insert(primaryRaw, v) table.insert(fallbackRaw, v) end
    end

    -- 3. Старая ссылка на words_two.lua
    local successWordListTwo, WordList_Two = pcall(function()
        return loadstring(game:HttpGet("https://raw.githubusercontent.com/jjengu/scripts/refs/heads/main/wordbomb/words_two.lua"))()
    end)
    if successWordListTwo and type(WordList_Two) == "table" then
        for _, v in pairs(WordList_Two) do table.insert(primaryRaw, v) table.insert(fallbackRaw, v) end
    end

    -- 4. Локальные хардкод слова
    local WordList_Three = {
        ...
        "counterdemonstrations", "characteristically",
        "unconstitutionality", "intercontinental",
        "phenomenological", "oversimplification",
        "disenfranchisement", "incompatibilities",
        "thermodynamically", "magnetohydrodynamics",
        "uncharacteristically", "electroencephalogram",
        "counterrevolutionary", "internationalization",
        "counterproductive", "environmentalists",
        "telecommunications", "immunohistochemistry",
        "hypercholesterolemia", "incomprehensibilities",
        "deinstitutionalization", "psychopharmacological",
        "reconceptualizations", "reindustrialization",
        "spectroscopically", "standardizations",
        "vulnerabilities", "westernization", "xenotransplantation",
        "sphenopalatineganglioneuralgia", "duos", "duo", 
        "immunochemesties", "army", "haha", "hah",
    }
    for _, v in pairs(WordList_Three) do
        table.insert(primaryRaw, v)
        table.insert(fallbackRaw, v)
    end

    -- 5. Мега-словарь на 286к с Render как фоллбэк
    local okDict, rawDict = pcall(function() return game:HttpGet(dictUrl) end)
    if okDict and rawDict then
        for word in rawDict:gmatch("[^\r\n]+") do
            table.insert(fallbackRaw, word)
        end
    end

    if #fallbackRaw == 0 then
        fallbackRaw = primaryRaw
    end
    if #primaryRaw == 0 then
        table.insert(primaryRaw, "apple")
    end
    ...
    print("✅ [UnpatchaBomb]: All lists & Render DB successfully loaded & indexed!")
end)
...
-- Функция поиска с приоритетом на старые пулы и фоллбэком на 286к
local function FindWordAuto(prompt)
    ...
    -- 1. Сначала ищем в старых классических пулах
    local found = searchInIndex(PrimaryPromptIndex)
    
    -- 2. Если там пусто, идем во все 286к с Render
    if not found then
...
MakeIntroLabel(0.54, 0.08, "PRO EDITION (ALL LISTS)")
...
local function ApplyTheme(theme)
    ...
    if theme == "Patriotic" then
        ...
        HowToUseText.Text = "Patriotic UI Active (All Lists)"
        T1.TextColor3 = Color3.fromRGB(255, 255, 255)
        ...
        HowToUseText.TextColor3 = Color3.fromRGB(140, 140, 140)
        HowToUseText.Text = "Modern UI Active (All Lists)"
        T1.TextColor3 = Color3.fromRGB(255, 255, 255)
        T2.TextColor3 = Color3.fromRGB(200, 200, 200)
        MinimizeButton.TextColor3 = Color3.fromRGB(255, 255, 255)
    end
...
if Games then
    ...
    if registerGame then
        registerGame.OnClientEvent:Connect(function(int)
            ...
            UsedWords = {}
            LastWord  = ""
            SetStatus("Game registered — ID: " .. tostring(int))
